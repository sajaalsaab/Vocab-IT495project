...
def keep_going():
    ...
    """
    flask.session['vocab']=WORDS.as_list()
...
def check():
    ...
    response = {'success': False, 'message': '','matches':matches,'count':flask.session["target_count"]}
    ...
    if matched and in_jumble and not (text in matches):
        ...
        response['message'] = f'تم ايجاد الكلمة'
        
        
    
    elif text in matches:

        response['message'] = f'تم العثور على ({text}) بالسابق'

    elif not in_jumble:

        response['message'] = f'الكلمة ({text}) لا يمكن انشائها من قائمة الحروف المعطاة ({jumble})'
    elif not matched:

        response['message'] = f'الكلمة ({text}) ليست ضمن قائمة الكلمات'
